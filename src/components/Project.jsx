  ...
  const renderRow = (list, ref, reverse = false) => {
    ...
    return (
      ...
      >
        ...
        {/* Scroll content */}
        <div className="scroll-content flex gap-2 w-max cursor-pointer">
          {duplicated.map((project, idx) => (
            <div
              key={idx}
              className="min-w-[200px] h-[150px] bg-gray-800 rounded flex items-center justify-center text-white shadow-lg"
            >
              <img
    ...
    <div className='mx-4 md:mx-16 lg:mx-44 text-white my-10 md:my-16 overflow-hidden space-y-6 scroll-smooth' id='project'>
      ...
      <div className="block md:hidden space-y-4">
        {projects.slice(0, 5).map((project, idx) => (
          <div key={idx} className="w-full h-[150px] bg-gray-800 rounded">
            <img
              src={project.projectImage}
              alt={project.projectName}
