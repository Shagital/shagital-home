...
const Navbar = () => {
  ...
  const navLinks = [
    { name: "PROJECTS", href: "#project", isLink: false },
  ...
  return (
    <div className="m-4 md:mx-7 md:my-7 lg:mx-9 lg:my-9 text-white scroll-smooth">
      <nav className="backdrop-blur-3xl bg-white/10 flex items-center justify-between rounded-full px-4 py-3 md:px-6">
        ...
        <div className="hidden md:flex gap-6 lg:gap-12">
          {navLinks.map((link) => (
            <div key={link.name}>
              <a
                href={link.href}
                className="text-sm text-white hover:underline hover:text-[#DC8B00] transition duration-200 cursor-pointer"
              >
                {link.name}
              </a>
            </div>
        ...
        <button className="hidden md:block text-sm px-3 py-2 border rounded-full border-[#DC8B00] cursor-pointer hover:bg-[#DC8B00] transition duration-500 hover:text-white">
          CONTACT US
        </button>
        <div
          className="block md:hidden text-sm px-1.5 py-1.5 border rounded border-[#DC8B00] cursor-pointer"
